...
document.addEventListener('DOMContentLoaded' , function () {
  let elementImgeOrder = document.querySelector('.item-order__image img') ;
  if (localStorage.getItem('img')) {
    elementImgeOrder.src = localStorage.getItem('img')
  } else {
    document.querySelector('.order-checkout').style.display = localStorage.getItem('display') ;
  } 
})
...
for (let i = 0 ; i < elementNOneOrder.length ; i++ ) {
 ...
 function functioneApply(a , jo ) {
       return function ( ...args) {
          a.apply(jo  , args) ;
       }
      
 }
 function functionLonge() {
  console.log(`Information: ${this.name} , ${this.age} `)
 }
 const elementObject = {
  name: 'Boris' , 
  age: 23  
 }
const elementApply =  functioneApply(functionLonge , elementObject ) ; 
elementApply(elementObject)
